pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/Shraddha4768/Ekart.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t shraddha4768/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u shraddha4768 -p ${dockerhub-pwd}'}
                   sh 'docker push shraddha4768/ekart:latest'
                }
            }
        }
